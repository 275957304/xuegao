function Ship(ctx){
	...
	this.setPosition = function(event){
		...
		if(this.left>this.clientWidth-this.width){
		...
		timer = setInterval(function(){
            if(direction.left){
				if(_this.left<0){
					...
					_this.left = _this.left - 10;
				}
            }
            else if(direction.right){
				if(_this.left > _this.clientWidth-_this.width){
					_this.left = _this.clientWidth-_this.width;
					//clearInterval(timer);
				}else{
					_this.left = _this.left + 10;
				}
...
function Food(type, left, id){
	...
	this.left = left;
	this.top = -50;
	this.speed = 0.04 * Math.pow(1.2, Math.floor(gameMonitor.time/this.speedUpTime)); //速度
	this.loop = 0;

	var p = this.type == 0 ? 'static/img/food1.png' : 'static/img/food2.png';
	// var p = 'static/img/food2.png';
...
var gameMonitor = {  //游戏监控
	...
	rollBg : function(ctx){  //roll 滚
		// if(this.bgDistance>=this.bgHeight){   //背景位置 >= 背景高度                      ctx.drawImage(img,x,y,width,height);  1开始剪切    2被剪切图像的宽度    3在画布上放置图像的 x 坐标位置
		// 	this.bgloop = 0;
		// }
	...
	run : function(ctx){
		...
		ctx.clearRect(0, 0, _this.bgWidth, _this.bgHeight);   // clearRect 在给定矩形内清空一个矩形：
		_this.rollBg(ctx);
